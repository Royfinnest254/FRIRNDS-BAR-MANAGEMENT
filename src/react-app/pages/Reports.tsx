...
export default function Reports() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      // Fetch data
      const [salesRes, inventoryRes, dailyStockRes] = await Promise.all([
        supabase.from("sales").select("*").order("sale_date", { ascending: true }),
        supabase.from("inventory").select("*, products(*)").order("quantity", { ascending: true }),
        supabase.from("daily_stock_records").select("*, products(*)").order("date", { ascending: true }),
      ]);
      ...
      // Flatten items
      const flattenedItems = (inventoryRes.data || []).map((item: any) => ({
        ...item,
        item_name: item.products?.name || "Unknown",
        price: item.products?.selling_price || 0
      }));

      // Flatten daily stocks if needed
      const flattenedDaily = (dailyStockRes.data || []).map((record: any) => ({
        ...record,
        item_name: record.products?.name || "Unknown"
      }));

      setSales(salesRes.data || []);
      setItems(flattenedItems);
      setDailyStocks(flattenedDaily);
    } catch (error) {
  ...
  const paymentData = useMemo(() => {
    const mpesa = sales.filter(s => s.payment_method === 'M-Pesa').reduce((sum, s) => sum + Number(s.total), 0);
    const cash = sales.filter(s => s.payment_method === 'Cash').reduce((sum, s) => sum + Number(s.total), 0);
    return [
  ...
  const stockVelocity = useMemo(() => {
    ...
    return items.map(item => ({
      ...
      status: (soldCounts[item.item_name] || 0) > 10 ? 'Fast' : 'Slow'
  ...
  const profitData = useMemo(() => {
    const itemProfits = dailyStocks.reduce((acc, record) => {
      if (record.profit_margin) {
        ...
        acc[name].total += record.profit_margin;
        acc[name].count += 1;
      }
      return acc;
